...
def draw():
    ...
    title=TITLE.render("Dev's HangMan",1,(0,0,0,0))
    win.blit(title,(WIDTH/2 -title.get_width()/2, 10)) # Title in center and then y axis= 24
    ...
    for event in pygame.event.get(): # Triggering the event
        ...
        if event.type==pygame.MOUSEBUTTONDOWN:
            ...
            for letter in letters:
                ...
                if visible:
                    dist = math.sqrt((x - x_mouse) ** 2 + (y - y_mouse) ** 2)  # to handle collision and to click the button exactly in the circle
